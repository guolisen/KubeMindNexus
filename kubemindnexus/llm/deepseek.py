"""Deepseek LLM implementation for KubeMindNexus."""
...
class DeepseekLLM(BaseLLM):
    ...
    def __init__(
        ...
        """
        super().__init__(LLMProvider.DEEPSEEK, model, api_key, base_url, parameters)
        ...
        try:
            ...
            if "tool_calls" in message:
                for tool_call in message["tool_calls"]:
                    ...
                    tool_call_data = {
                        "id": tool_call.get("id", f"deepseek-tool-{len(tool_calls)}"),
    ...
    async def generate_stream(
        ...
        """Generate a response from the Deepseek LLM with streaming support.
        ...
        try:
            ...
            logger.error(f"HTTP error streaming from Deepseek: {e.response.status_code} - {str(e)}")
            yield f"Error streaming response: HTTP {e.response.status_code}"
        except httpx.RequestError as e:
            logger.error(f"Request error streaming from Deepseek: {str(e)}")
            yield f"Error streaming response: Connection error"
        except Exception as e:
            logger.error(f"Error streaming response from Deepseek: {str(e)}")
    ...
    async def generate_with_tools(
        ...
        """Generate a response from the Deepseek LLM with tool definitions.
        ...
        try:
            ...
            logger.error(f"Error generating response from Deepseek with tools: {str(e)}")
            return f"Error generating response: {str(e)}", []
